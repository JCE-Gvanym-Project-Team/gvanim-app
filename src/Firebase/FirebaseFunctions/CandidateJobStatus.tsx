...
export class CandidateJobStatus {
  ...
  public async addRecomendation(
    fullName: string,
    phone: string,
    eMail: string,
    recomendation: string
  ) {
    if (this._recomendations.length >= 10) return;
    if (this._recomendations.map((rec) => rec._phone).includes(phone)) return;
    this._recomendations.push(
      new Recomendation(fullName, phone, eMail, recomendation)
    );
  ...
  public editInterviewSummery(summery: string, index: number) {
    ...
    else this._interviewsSummery[index] = summery;
  }
  public async edit(
    candidateId: string = "",
    about: string = "",
    ...
    if (candidateId.length > 0) this._candidateId = candidateId;
    if (about.length > 0) this._about = about;
    if (matchingRate >= 0) this._matchingRate = matchingRate;
    replaceData(await this.getPath(), this);
  }
  private async getPath() {
    ...
    candidateIds.forEach(async (id) => {
      if (
        (await getObjectAtPath("/CandidatesJobStatus/" + id))._candidateId ===
          this._candidateId &&
        (await getObjectAtPath("/CandidatesJobStatus/" + id))._jobNumber ===
          this._jobNumber
      )
        firebaseId = id;
    });
  ...
  public async add() {
    ...
    else console.log("the CandidatesJobStatus already exists");
  }
}
async function getCandidateJobStatusFromDatabase(): Promise<
  CandidateJobStatus[]
> {
  try {
...
async function removeCandidateJobStatus(
  candId: string = "",
  jobNumber: number = -1
) {
  let candidateIds = await getFirebaseIdsAtPath("/CandidatesJobStatus");
...
export async function getFilteredCandidateJobStatuses(
  attributes: string[] = [],
  values: string[] = [],
  sortBy: string = ""
) {
  ...
  if (sortBy === "jobNumber") return candidateJobStatuses.sort(sortByJobNumber);
  if (sortBy === "candidateId")
    return candidateJobStatuses.sort(sortByCandidateId);
  if (sortBy === "status") return candidateJobStatuses.sort(sortByStatus);
  if (sortBy === "matchingRate")
    return candidateJobStatuses.sort(sortByMatchingRate);
  if (sortBy === "applyDate") return candidateJobStatuses.sort(sortByApplyDate);
...
function sortByCandidateId(
  a: CandidateJobStatus,
  b: CandidateJobStatus
): number {
